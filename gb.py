...
def format_brazilian(value, format_string):
    ...
    if pd.isna(value) or value == float('inf') or value == float('-inf'):
        return "N/A"
    try:
        # Formata o número como string e depois substitui os separadores.
        formatted_value = format_string.format(value)
        # Troca temporária para não confundir vírgula e ponto.
...
def clean_and_round_payload(d):
    ...
    if isinstance(d, dict):
        return {k: clean_and_round_payload(v) for k, v in d.items()}
    if isinstance(d, list):
        return [clean_and_round_payload(i) for i in d]
    if isinstance(d, float) or isinstance(d, np.floating):
        if pd.isna(d) or np.isinf(d):
            return 0.0
        return round(float(d), 2)
    if isinstance(d, str) and d == 'inf':
        return 0.0
...
def fetch_data_from_bigquery(project_gcp_id, dataset_id, table_id,
    ...
    try:
        ...
        for col in integer_cols:
             if col in df_renamed.columns:
                df_renamed[col] = pd.to_numeric(df_renamed[col], errors='coerce').fillna(0).astype(int)

        string_cols_to_convert = [
            'utm_source_pl', 'utm_campaign_pl', 'utm_term_pl', 'campaign_id_gads',
            'keyword_text_gkw', 'campaign_name_gkw', 'ad_group_name_gkw',
            'ad_id_gkw', 'campaign_name_gads', 'ad_id_fb', 'Criativo_fb_ad_name',
            'adset_name_fb', 'campaign_name_fb'
        ]
...
<style>
    ...
    /* --- AJUSTE GLOBAL PARA RESPONSIVIDADE --- */
    /* Permite que as colunas do Streamlit se empilhem em telas menores */
    ...
    /* Estilo para os cartões de métrica (st.metric) das seções GAds/Meta */
    ...
    /* --- Grade de KPIs Responsiva para "Performance Geral" --- */
    .kpi-grid-container {
        display: grid;
        grid-template-columns: repeat(4, 1fr); /* Padrão: 4 colunas */
        gap: 16px; /* Espaçamento vertical e horizontal */
    }
...
if not df_p2_metas_original.empty and 'data_meta_p2' in df_p2_metas_original.columns:
    mask_p2 = (df_p2_metas_original['data_meta_p2'].dt.date >= data_selecionada_inicio) & \
            (df_p2_metas_original['data_meta_p2'].dt.date <= data_selecionada_fim)
    df_p2_conferidor = df_p2_metas_original[mask_p2]


# --- CÁLCULO PERFORMANCE GERAL E KPIs ---
metricas_config = {
    "Faturamento": {"real_col_pl": "real_faturamento_venda_pl", "meta_col": "meta_faturamento_p2", "acumulada": True, "formato": "R$ {:,.2f}", "regra_status": {"ruim_ate": 74.99, "atc_ate": 99.99, "bom_acima": 100, "tipo": "maior_melhor"}},
    "Investimento": {"meta_col": "meta_investimento_p2", "acumulada": True, "formato": "R$ {:,.2f}", "regra_status": {"bom_entre": (95, 105), "atc_entre_inf": (85, 94.99), "atc_entre_sup": (105.01, 115), "ruim_fora": True, "tipo": "dentro_faixa_invest"}},
    "Leads": {"real_pl_count": True, "meta_col": "meta_leads_p2", "acumulada": True, "formato": "{:,.0f}", "regra_status": {"ruim_ate": 79.99, "atc_ate": 95.99, "bom_acima": 96, "tipo": "maior_melhor"}},
...
if not df_pl_filtrado_intervalo.empty:
    ...
    for m_key_pl in ["Faturamento", "Leads", "MQL", "Vendas"]:
...
if not df_fb_ads_bq.empty and 'Investido_fb_val' in df_fb_ads_bq.columns:
    investimento_total += df_fb_ads_bq['Investido_fb_val'].sum()
...
investimento_real = realizados_calculados["Investimento"]
...
for metrica, valor_real in realizados_calculados.items():
    if metrica in df_tabela_performance.columns:
        df_tabela_performance.loc["Realizado", metrica] = valor_real

for metrica_nome, config in metricas_config.items():
    meta_nome = config.get("meta_col")
    valor_meta_periodo = 0.0
    if meta_nome and not df_p2_metas_original.empty and meta_nome in df_p2_metas_original.columns and 'project_id_p2' in df_p2_metas_original.columns and 'ano_mes' in df_p2_metas_original.columns:
        ...
        if not metas_projeto_df.empty:
            if config["acumulada"]:
                ...
                meta_mes_especifico_df = metas_projeto_df[metas_projeto_df['ano_mes'] == ultimo_mes_filtro]
                if not meta_mes_especifico_df.empty:
                    valor_meta_periodo = meta_mes_especifico_df[meta_nome].iloc[0]

    df_tabela_performance.loc["Meta", metrica_nome] = valor_meta_periodo
    real = df_tabela_performance.loc["Realizado", metrica_nome]
    meta = df_tabela_performance.loc["Meta", metrica_nome]
    df_tabela_performance.loc["Diferença", metrica_nome] = real - meta
    percent_atingido = 0.0
    tipo_regra = config.get("regra_status", {}).get("tipo", "maior_melhor")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if presentation_link:
                        ...
                        st.session_state.presentation_link = presentation_link
                        st.session_state.success_title = success_title
                        st.rerun()
                    else:
                        st.error("Resposta recebida, mas o link da apresentação não foi encontrado.")
                        st.json(response_data)
                except json.JSONDecodeError:
                    st.error("A automação funcionou, mas a resposta não foi um JSON válido.")
                    st.text(response.text)
            else:
                st.error(f"Erro ao comunicar com o webhook: {response.status_code}")
                st.text(response.text)
...
with btn_col1:
    st.write("")
    st.write("")
    if st.button("Check-in", key="btn1"):
        trigger_webhook(WEBHOOK_CHECKIN, payload_data_checkin, "Check-in", "Concluído!")
with btn_col2:
    st.write("")
    st.write("")
    if st.button("Relatório", key="btn2"):
        trigger_webhook(WEBHOOK_RELATORIO, payload_data_checkin, "Relatório", "Concluído!")
with btn_col3:
    st.write("")
    st.write("")
...
if st.session_state.get("show_success_dialog", False):
    ...
    def show_link_dialog():
        ...
        if st.button("Fechar", key="close_dialog"):
            ...
            st.session_state.presentation_link = ""
            st.session_state.success_title = ""
            st.rerun()
    show_link_dialog()
...
for metrica in kpi_order:
    if metrica not in df_tabela_performance.columns:
        ...
        """
        kpi_cards_html.append(card_html)
        continue
...
# Renderiza a grade de KPIs de uma só vez
html_grid = f'<div class="kpi-grid-container">{"".join(kpi_cards_html)}</div>'
...
if not df_fb_creatives_final.empty:
    ...
    kpis_fb_data['Total de vendas'] = df_meta_ads_agg_kpi['Vendas'].sum()
    kpis_fb_data['Faturamento'] = df_meta_ads_agg_kpi['Valor'].sum()
